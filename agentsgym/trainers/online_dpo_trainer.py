"""Official implementation of Online DPO - just here for reference - we will not use inbuilt online DPO because of the following reasons (as answered by ChatGPT)


You certainly *can* use `trl.OnlineDPOTrainer`—it works out-of-the-box for the **canonical** online-DPO recipe (prompt-only dataset → sample 2 completions with `model.generate` → score them with *one* reward model or judge).  
We wrote a bespoke trainer only because your workflow has requirements that the stock class doesn’t cover yet:

| Requirement in your pipeline | How stock `OnlineDPOTrainer` behaves | Why we rolled our own |
|------------------------------|--------------------------------------|-----------------------|
| **Environment–based generation** (the `agentsgym.envs.Environment` decides how to call vLLM, inject tools, etc.) | `_generate` / `_generate_vllm` call the policy’s `generate` directly; no hook for an external env | We need `env.generate( …, agent=DirectOutputAgent, …)` so the environment can do game logic, tool calls, GM reasoning, etc. |
| **Arbitrary Python reward functions (possibly several, with weights)** | Exactly *one* of `reward_model` *or* `judge` must be supplied; any other combination raises `ValueError` | Your code mixes neural rewards with hand-crafted heuristics, then linearly combines them—unsupported in the stock trainer. |
| **Message-dict prompts & custom chat templates** | Expects plain strings (or HF “conversational” format) and its own template handling. | We already template prompts inside the env, so we bypass the trainer’s template logic. |
| **Shared vLLM engine across workers** | Stock class reloads weights into its private vLLM instance every step; no reuse of an existing engine. | We reuse the env’s long-lived vLLM client to avoid the load→generate→eject overhead. |
| **Custom logging (group-normalised rewards, per-func metrics, Rich console preview, WandB tables aligned with GRPO logs)** | Provides generic reward / accuracy stats only. | We log the same metrics schema that your GRPO runs already expect. |

### When the built-in trainer *is* enough

If your training loop really is:

```text
prompt-only dataset
      ↓
sample 2 completions from the policy
      ↓
score them with a single reward model or PairRMJudge
      ↓
apply DPO loss
```

…then `OnlineDPOTrainer` + `OnlineDPOConfig` is simpler—just point it at a tokenizer, a reward model or judge, and optionally enable `use_vllm=True` as in the HF quick-start docs.

### If you want to stay closer to upstream

Another middle ground is to **sub-class** `OnlineDPOTrainer` instead of writing everything from scratch:

```python
from trl import OnlineDPOTrainer

class EnvAwareOnlineDPO(OnlineDPOTrainer):
    def __init__(self, env: Environment, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.env = env

    # override just the bits you need
    def _generate(self, model, prompts):
        return self.env.generate(prompts, llm=self.vllm_client, sampling_params=self.generation_config)
```

That gives you HF maintenance for everything else (deepspeed, PEFT, distributed, logging) while still letting the environment drive generation and rewards.

---

**Bottom line:** choose the stock `OnlineDPOTrainer` if its assumptions match your setup; otherwise, extend or recreate it (as we did) so the trainer fits the *environment-first, multi-reward* architecture you already have. 
"""
