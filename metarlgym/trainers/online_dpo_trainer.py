...
|------------------------------|--------------------------------------|-----------------------|
| **Environment–based generation** (the `metarlgym.envs.Environment` decides how to call vLLM, inject tools, etc.) | `_generate` / `_generate_vllm` call the policy’s `generate` directly; no hook for an external env | We need `env.generate( …, agent=DirectOutputAgent, …)` so the environment can do game logic, tool calls, GM reasoning, etc. |
